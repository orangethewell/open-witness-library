...
use aes::Aes128;
use cbc::{cipher::{KeyIvInit, BlockDecryptMut, generic_array::GenericArray, BlockSizeUser}, Decryptor};
...
fn decrypt_aes_128_cbc(key: &[u8], iv: &[u8], data: &[u8]) -> Vec<u8> {
    ...
    let mut blocks: Vec<GenericArray<u8, typenum::U16>> = buffer.chunks_exact(block_size)
...
impl PubCatalog {
    ...
    pub fn install_publication<T: Into<PathBuf>>(&self, pub_path: T) -> CatalogResponse {
        ...
        if !Path::new(&self.local.join(PathBuf::from_iter(["publications", &lang, &category]))).exists() {
            fs::create_dir_all(&self.local.join(PathBuf::from_iter(["publications", &lang, &category]))).expect("Failed to create directory")
        }

        publication_dir.push(self.local.join(PathBuf::from_iter(["publications", &lang, &category, &filename.replace(".jwpub", "")])));
    ...
    pub fn get_list_from_category(&self, lang: String, category: String, start_idx: Option<usize>, limit: Option<usize>) -> Vec<Publication> {
        let pub_list_target = self.local.join(PathBuf::from_iter(["publications", &lang, &category]));
        ...
        for entry in fs::read_dir(&pub_list_target).unwrap().skip(start_idx.or(Some(0)).unwrap()).take(std::cmp::min(limit.or(Some(25)).unwrap(), 25)) {
            ...
            list.push(Publication {
                ...
                cover_icon_path: PathBuf::from(
                        entry.path().join(PathBuf::from_iter(["content", manifest["publication"]["images"][1]["fileName"].as_str().unwrap()]))
                    ),
    ...
    pub fn get_summary_from(&self, lang: String, category: String, pub_symbol: String) -> Result<Vec<Chapter>, ()> {
        ...
        if let Ok(connection) = sqlite::open(&pub_directory.join(PathBuf::from_iter(["content", manifest["publication"]["fileName"].as_str().unwrap()]))){
    ...
    pub fn get_chapter_content(&mut self, lang: String, category: String, pub_symbol: String, chapter_id: i64) -> ChapterContent {
        ...
        if let Ok(connection) = sqlite::open(&pub_directory.join(PathBuf::from_iter(["content", manifest["publication"]["fileName"].as_str().unwrap()]))) {
            ...
            while let Some(Ok(row)) = cursor.next() {
                ...
                let master_key = self.cached_key.get(&pub_symbol).cloned().unwrap_or_else(|| {
                ...
                let decrypted_content = inflate_bytes_zlib(&decrypt_aes_128_cbc(
                    key,
                    iv,
                    &encrypted_content
                )).unwrap();
    ...
    pub fn get_cover_icon(&mut self, lang: String, category: String, pub_symbol: String) -> Vec<u8>  {
        ...
        if let Ok(data) = fs::read(&pub_directory.join(PathBuf::from_iter(["content", manifest["publication"]["images"][0]["fileName"].as_str().unwrap()]))) {
            return data;
        } else {
            return vec![];
        }
    ...
    fn normalize_request_directory(&self, lang: &str, category: &str, pub_symbol: &str) -> PathBuf {
        self.local.join(PathBuf::from_iter(["publications", lang, category, pub_symbol]))
    } 
